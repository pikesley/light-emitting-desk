...
class TestConverge(TestCase):
    ...
    def test_easy_converge(self):
        """Test the simple case."""
        client = app.test_client()
        data = json.dumps({"colour": [123, 123, 0]})
        response = client.post("/desk/all/converge", headers=headers, data=data)
    ...
    def test_get_mode(self):
        ...
        client.post("/desk/all/converge", headers=headers, data=data)
        response = client.get("/desk/mode")

        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            json.loads(response.data),
            {"mode": "converge", "status": "OK"},
        )
